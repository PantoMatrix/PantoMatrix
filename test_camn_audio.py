...
def inference(model, audio_path, device, save_path, sr, pose_fps, seed_frames):
...
def main():
    ...
    for audio_path in tqdm(audio_files, desc="Inference"):
        all_t += inference(model, audio_path, device, args.save_path, sr, pose_fps, seed_frames)
    print(f"generate total {all_t/pose_fps:.2f} seconds motion in {time.time()-start_time:.2f} seconds, saved in {args.save_path}")
    
    start_time = time.time()
    if args.visualization:
        for audio_path in tqdm(audio_files, desc="Visualize"):
            visualize_one(args.save_path, audio_path)
        print(f"render total {all_t/pose_fps:.2f} seconds motion in {time.time()-start_time:.2f} seconds, saved in {args.save_path}")
